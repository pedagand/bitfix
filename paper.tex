...
compiler writers, for whom job
security~\citep{bernstein:death-compiler,venkatesh:compiler-security}
has become a major concern. Luckily for us, every now and then,
someone inadvertently let the genies out of their Klein bottle and we
get to read about the subtle art of ``bitslicing''~\citep{biham:DES}
and ``fixslicing''~\citep{adominicai:fixslicing-gift,
...
timing~\citep{bernstein:timing-attack-AES}), this pipeline can be more
precisely understood as a combinational circuit. We are forbidden from
...
modern ciphers operate over a certain matrix (oftentimes, a 2D or 3D
array of bits): the cipher is not specified over a flat sequence of
individual bits but over a n-dimensional array of bits.
...
high-throughput, ciphers are typically designed so as to be admit a
...
sole purpose of the ``Section 5'' scene to exhibit a bitsliced witness
...
programming~\citep{bird:squiggol,roconnor:functor-oriented}. Because our
government-approved programming language does not yet support ad-hoc
polymorphism~\citep{white:modular-implicit}, the French community may be
...
\texttt{autorewrite} tactics to emulate a transformation-based simplification
engine~\citep{peyton-jones:haskell-optim} (part of an imaginary optimizing
compiler).
...
defined over any Boolean algebra~\citep{givant:boolean-algebras}:
%
...
functor~\citep{yorgey:typeclassopedia}:
%
...
\begin{prop}
  ...
  %
  and can be implemented reasonably efficiently.
\end{prop}
...
the answers, kindly provided by~\citet{adominicai:fixslicing-gift}, and crawled our way
...
technique~\citep{adominicai:fixslicing-gift}. Gift is specified in
