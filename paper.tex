...
cryptographic primitives can be distilled in a few paragraphs. A
...
is a trade-off between latency (fewer iterations means faster
...
To avoid side-channel attacks (for example, based on
...
Because OCaml, our favorite  programming language, does not yet
...
%
\coqfrom{Algebra}{ApplicativeComm}
...
boolean takes up a full register, a 4-bytes
...
remains suboptimal. We can further increase register usage and
...
next section, we present a clever trick to get permutations to
nearly vanish.
...
Note that we have to transform the input key beforehand, so as to
